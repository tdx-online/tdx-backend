...
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `product_properties_view` AS select `online_store`.`property_value`.`id` AS `id`,`tab`.`id` AS `pid`,`tab`.`ptid` AS `ptid`,`tab`.`name` AS `name`,`online_store`.`property_value`.`value` AS `value` from (((select `online_store`.`product`.`id` AS `id`,`online_store`.`property`.`name` AS `name`,`online_store`.`property`.`id` AS `ptid` from (`online_store`.`property` left join `online_store`.`product` on((`online_store`.`product`.`cid` = `online_store`.`property`.`cid`))))) `tab` left join `online_store`.`property_value` on(((`tab`.`id` = `online_store`.`property_value`.`pid`) and (`tab`.`ptid` = `online_store`.`property_value`.`ptid`))));
